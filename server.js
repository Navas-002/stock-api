...
app.get('/stock/:ticker', async (req, res) => {
  ...
  try {
    ...
    res.json({ ticker, name: profile.name||ticker, price: quote.c, change: +(quote.c-quote.pc).toFixed(2), changePct: +(((quote.c-quote.pc)/quote.pc)*100).toFixed(2), high: quote.h, low: quote.l, open: quote.o, prevClose: quote.pc, marketCap: profile.marketCapitalization?`$${(profile.marketCapitalization/1000).toFixed(2)}T`:null, sector: profile.finnhubIndustry||null, exchange: profile.exchange||null });
